...
function onOpen() {
  ...
  ui.createMenu('Training Plan')
    ...
    .addSeparator()
    .addItem('Add Athlete...', 'addAthlete')
...
function ensureAthletesTab() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName('Athletes');
  if (sheet) return sheet;

  sheet = ss.insertSheet('Athletes');
...
function addAthlete() {
  ...
  const nameResponse = ui.prompt('Add Athlete (1/3)', 'Athlete name:', ui.ButtonSet.OK_CANCEL);
  if (nameResponse.getSelectedButton() !== ui.Button.OK) return;
  const name = nameResponse.getResponseText().trim();
  if (!name) { ui.alert('Name is required.'); return; }

  const idResponse = ui.prompt('Add Athlete (2/3)',
    'intervals.icu Athlete ID (e.g., i12345):\n\nThe athlete can find this in their intervals.icu profile URL.',
    ui.ButtonSet.OK_CANCEL);
  if (idResponse.getSelectedButton() !== ui.Button.OK) return;
  const athleteId = idResponse.getResponseText().trim();
  if (!athleteId) { ui.alert('Athlete ID is required.'); return; }

  const suggestedTab = name.split(' ')[0] + ' - Plan';
  const tabResponse = ui.prompt('Add Athlete (3/3)',
    'Plan tab name for this athlete:\n\n' +
    'If it doesn\'t exist yet, it will be created from the Template tab\n' +
    'with dates starting from next Monday.\n\n' +
    'Suggested: ' + suggestedTab,
    ui.ButtonSet.OK_CANCEL);
  if (tabResponse.getSelectedButton() !== ui.Button.OK) return;
  const tabName = tabResponse.getResponseText().trim() || suggestedTab;

  // Add to roster
  const athletesSheet = ensureAthletesTab();
  const nextRow = athletesSheet.getLastRow() + 1;
  athletesSheet.getRange(nextRow, 1, 1, 4).setValues([[name, athleteId, tabName, '']]);

  // Create plan tab from Template if it doesn't exist
  if (!ss.getSheetByName(tabName)) {
    const template = ss.getSheetByName('Template Training Plan') || ss.getSheetByName('Template');
    if (template) {
      const newSheet = template.copyTo(ss);
      newSheet.setName(tabName);
      personalizeAthleteTab(newSheet, name);
      ui.alert('Athlete Added',
        `${name} added to roster.\n\n` +
        `Plan tab "${tabName}" created from Template with dates starting next Monday.\n` +
        `Update the plan and sync when ready.`,
        ui.ButtonSet.OK);
    } else {
      ui.alert('Athlete Added',
        `${name} added to roster.\n\n` +
        `No Template tab found — create "${tabName}" manually.`,
        ui.ButtonSet.OK);
    }
  } else {
    ui.alert('Athlete Added',
      `${name} added to roster.\n\nPlan tab "${tabName}" already exists.`,
      ui.ButtonSet.OK);
  }
}

function personalizeAthleteTab(sheet, athleteName) {
  ...
  for (let i = 0; i < data.length; i++) {
    const cellB = (data[i][1] || '').toString();

    // Update title row with athlete name
    if (/training plan/i.test(cellB)) {
      sheet.getRange(i + 1, 2).setValue(athleteName + ' Training Plan');
      continue;
    }

    // Update week headers with new dates
    if (/week\s+\d+/i.test(cellB) && /\d+\s+\w+\s*-/.test(cellB)) {
      const weekStart = new Date(nextMonday);
      weekStart.setDate(nextMonday.getDate() + (weekCount * 7));
      ...
      sheet.getRange(i + 1, 2).setValue('Week ' + weekNum + '\n' + startStr + ' - ' + endStr);
      weekCount++;
      continue;
    }

    // Rename coach-specific notes rows (e.g., "Lee' Notes", "Lee's Notes")
    // but not "Template' Notes", "Session Notes", or "Your Notes"
    const cellBLower = cellB.toLowerCase();
    if (/notes$/i.test(cellB) &&
        !cellBLower.includes('session') &&
        !cellBLower.includes('your') &&
        !cellBLower.includes('template')) {
      sheet.getRange(i + 1, 2).setValue("Coach's Notes");
    }
  }
}
...
function getAthleteRoster() {
  ...
  if (!sheet) {
    throw new Error('Athletes tab not found. Use Training Plan > Add Athlete to get started.');
  }
...
function syncAllAthletes() {
  ...
  // Dry-run pass to count events per athlete
  const previews = [];
  let totalEvents = 0;
  for (const athlete of roster) {
    const preview = syncAthlete(athlete.name, athlete.id, athlete.tab, weekFilter, true, athlete.row);
    const count = preview.success ? preview.eventCount : 0;
    previews.push({ athlete: athlete, count: count, error: preview.success ? null : preview.error });
    totalEvents += count;
  }

  const weekLabel = weekFilter ? ` (Week ${weekFilter})` : ' (All weeks)';
  const athleteLines = previews.map(p => {
    if (p.error) return `• ${p.athlete.name} — ERROR: ${p.error}`;
    return `• ${p.athlete.name} — ${p.count} events`;
  }).join('\n');

  const confirm = ui.alert('Sync All Athletes' + weekLabel,
    `${roster.length} athlete(s), ${totalEvents} total events:\n\n` +
    athleteLines +
    '\n\nProceed with upload?',
    ui.ButtonSet.YES_NO);
...
function syncSpecificAthlete() {
  ...
  if (rosterResult.roster.length === 0) {
    let msg = 'No valid athletes found in the Athletes tab.';
    if (rosterResult.errors.length > 0) {
      msg += '\n\nIssues found:\n' + rosterResult.errors.join('\n');
    }
  ...
  if (!preview.success) {
    ui.alert('Error', `${athlete.name}: ${preview.error}`, ui.ButtonSet.OK);
    return;
  }

  const weekLabel = weekFilter ? ` (Week ${weekFilter})` : '';
  const eventLines = preview.events.map(e =>
    `${e.start_date_local.substring(0, 10)} | ${e.name.substring(0, 35).padEnd(35)} | ${e.type}`
  ).join('\n');

  const confirm = ui.alert(`${athlete.name}${weekLabel} — ${preview.eventCount} events`,
    eventLines + '\n\nProceed with upload?',
    ui.ButtonSet.YES_NO);
  if (confirm !== ui.Button.YES) return;
...
function syncAthlete(athleteName, athleteId, tabName, weekFilter, dryRun, athleteRow) {
  ...
  events.forEach(e => {
    e.external_id = generateExternalId(athleteId, e.start_date_local, e.name, e.type);
    delete e.weekNumber;
  });
